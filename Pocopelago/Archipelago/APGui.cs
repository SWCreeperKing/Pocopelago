using Pocopelago;
using UnityEngine;
using static Pocopelago.Archipelago.ClownClient;
...
public class APGui : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        if (!Client.IsConnected && GUI.Button(new Rect(20 + Offset.x, 210 + Offset.y, 180, 30), "Connect"))
        {
            ...
            if (error is not null && error.Length > 0 && !(error.Length == 1 && error[0].Trim() is ""))
            {
                State = string.Join("\n", error);
                return;
            }
        ...
        Core.StartButton.SetActive(Client.IsConnected);
